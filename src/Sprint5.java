	...
	public static void main(String args[]) {
		...
		for (playerId = 0; playerId < GameData.NUM_PLAYERS_PLUS_NEUTRALS; playerId++) {
			...
			if (playerId == 1) {
				...
				players[playerId].setBot(new BottovonBotmarck(board, players[playerId]));
			}
			...
			do {
				...
				if (!ui.isTurnEnded()) {
					...
					if (board.isInvasionSuccess() && (board.isEliminated(defencePlayer.getId()))) {
						...
						ui.displayCardsWon(currPlayer, defencePlayer, cards); // No cards received
																				// from passive
																				// neutrals
